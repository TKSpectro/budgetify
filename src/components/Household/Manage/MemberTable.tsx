    ...
    owner.id !== member.id ? (
      ...
      <>
        <Button onClick={() => onRemoveMemberClickHandler(member)} className="mr-2">
          <UserRemoveIcon className="w-4 h-4 sm:w-6 sm:h-6" />
        </Button>
  ...
  return (
    <>
      ...
      <table className="table-fixed w-full break-words">
        <thead>
          <tr>
            <th className="w-1/3 hidden sm:table-cell">{t('common:name')}</th>
        ...
        <tbody className="divide-y divide-gray-200 text-center">
          {members.map((member: User) => {
            return (
              <tr key={member.id}>
                <td className="py-2">
                  <div className=" text-gray-800 dark:text-gray-100 my-4 hidden sm:block text-center">
                    {member.name}
                  </div>
                ...
                <td className="py-4 hidden sm:table-cell">{actionButtons(member)}</td>
              </tr>
            );
          })}
        </tbody>
