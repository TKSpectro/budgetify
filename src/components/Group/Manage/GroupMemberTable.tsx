...
const UPDATE_GROUP_OWNER_MUTATION = gql`
  mutation UPDATE_GROUP_OWNER_MUTATION($id: String!, $ownerId: String!) {
    updateGroup(id: $id, ownerId: $ownerId) {
      id
      ownerId
    }
  }
...
export default function GroupMemberTable({ members, owner, ...props }: Props) {
  const router = useRouter();
  const groupId = router.query.groupId as string;

  const [updateGroupMutation] = useMutation<MutationUpdateGroupArgs>(UPDATE_GROUP_OWNER_MUTATION, {
    onCompleted: () => {},
    onError: () => {},
  });
  ...
  const makeOwnerHandler = (id: string) => {
    updateGroupMutation({ variables: { id: groupId, ownerId: id } });
  };
